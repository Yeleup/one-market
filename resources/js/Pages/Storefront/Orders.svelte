...
{#snippet children()}
<div>
    <h1 class="mb-6 text-2xl font-semibold tracking-tight text-stone-900">Мои заказы</h1>
    ...
    {#if orders.data.length === 0}
        <div class="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p class="text-sm text-stone-500">Заказов пока нет</p>
        </div>
    {:else}
        <!-- Mobile: card layout -->
            ...
            {#each orders.data as order}
                <a href="/storefront/orders/{order.id}" class="block rounded-2xl border border-stone-200 bg-white p-4 transition-colors hover:border-stone-300">
                    <div class="mb-2 flex items-center justify-between">
                        <span class="text-sm font-medium text-stone-900">Заказ #{order.id}</span>
                    ...
                    <div class="flex items-center justify-between text-xs text-stone-400">
                        ...
                        <span class="font-medium text-stone-700">{order.total_bonus} бон.</span>
                    </div>
                </a>
            {/each}
        </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-stone-100">
                            ...
                            <th class="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-stone-400">Вес</th>
                        </tr>
                    </thead>
                    <tbody>
                        {#each orders.data as order}
                            <tr onclick={() => router.visit(`/storefront/orders/${order.id}`)} style="cursor: pointer;" class="border-t border-stone-50 transition-colors hover:bg-stone-50/50">
                                ...
                                <td class="px-4 py-3 text-right text-stone-400">{order.total_weight_grams} г</td>
                            </tr>
                        {/each}
                    </tbody>
                </table>
