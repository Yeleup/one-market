<script>
    ...
    function logout() {
        router.post(page.props.routes.logout);
    }
    ...
    function switchLanguage(languageCode) {
        ...
        router.post(page.props.routes.language, {
    ...
    >
        <div class="mx-auto max-w-6xl px-4 sm:px-6">
            <div class="flex h-16 items-center justify-between">
                <!-- Logo -->
                <a href={page.props.routes.catalog} class="text-lg font-semibold tracking-tight text-stone-900">
                    One Market
                </a>
                ...
                <nav class="hidden items-center gap-1 md:flex">
                    {#if page.props.auth?.client}
                        ...
                        <a
                            href={page.props.routes.orders}
                            class="rounded-lg px-3 py-2 text-sm font-medium text-stone-600 transition-colors hover:bg-stone-100 hover:text-stone-900"
                        >
                            {t('layout.nav.orders', 'Заказы')}
                        </a>
                        <a
                            href={page.props.routes.bonuses}
                            class="rounded-lg px-3 py-2 text-sm font-medium text-stone-600 transition-colors hover:bg-stone-100 hover:text-stone-900"
                        >
                ...
                <div class="hidden items-center gap-3 md:flex">
                    ...
                    {#if page.props.auth?.client}
                        <div class="flex items-center gap-3 border-l border-stone-200 pl-3">
                            ...
                            <a
                                href={page.props.routes.profile}
                    ...
                    {:else}
                        <a
                            href={page.props.routes.login}
                            class="rounded-lg px-3 py-2 text-sm font-medium text-stone-600 transition-colors hover:bg-stone-100 hover:text-stone-900"
                        >
                            {t('layout.auth.login', 'Войти')}
                        </a>
                        <a
                            href={page.props.routes.register}
        ...
        {#if mobileMenuOpen}
            <div class="border-t border-stone-100 bg-white md:hidden">
                <div class="space-y-1 px-4 pb-4 pt-3">
                    <a onclick={closeMobileMenu} href={page.props.routes.catalog} class="block rounded-lg px-3 py-2.5 text-sm font-medium text-stone-700 transition-colors hover:bg-stone-50">{t('layout.nav.catalog', 'Каталог')}</a>
                    {#if page.props.auth?.client}
                        <a onclick={closeMobileMenu} href={page.props.routes.dashboard} class="block rounded-lg px-3 py-2.5 text-sm font-medium text-stone-700 transition-colors hover:bg-stone-50">{t('layout.nav.dashboard', 'Кабинет')}</a>
                        <a onclick={closeMobileMenu} href={page.props.routes.orders} class="block rounded-lg px-3 py-2.5 text-sm font-medium text-stone-700 transition-colors hover:bg-stone-50">{t('layout.nav.orders', 'Заказы')}</a>
                        <a onclick={closeMobileMenu} href={page.props.routes.bonuses} class="block rounded-lg px-3 py-2.5 text-sm font-medium text-stone-700 transition-colors hover:bg-stone-50">{t('layout.nav.bonuses', 'Бонусы')}</a>
                        <a onclick={closeMobileMenu} href={page.props.routes.profile} class="block rounded-lg px-3 py-2.5 text-sm font-medium text-stone-700 transition-colors hover:bg-stone-50">{t('layout.nav.profile', 'Профиль')}</a>
                    ...
                    {:else}
                        <div class="flex gap-2 border-t border-stone-100 pt-3">
                            <a onclick={closeMobileMenu} href={page.props.routes.login} class="flex-1 rounded-lg border border-stone-200 px-3 py-2.5 text-center text-sm font-medium text-stone-700 transition-colors hover:bg-stone-50">
                                {t('layout.auth.login', 'Войти')}
                            </a>
                            <a onclick={closeMobileMenu} href={page.props.routes.register} class="flex-1 rounded-lg bg-stone-900 px-3 py-2.5 text-center text-sm font-medium text-white transition-colors hover:bg-stone-800">
                                {t('layout.auth.register', 'Регистрация')}
                            </a>
                        </div>
                    {/if}
